...
from qiskit_omelyan import Forest_Ruth4

from model import heisenbergXXZ

# This example demonstrates how to use the Forest_Ruth4 scheme to build
...
if __name__ == "__main__":
    ...
    forest_ruth4 = Forest_Ruth4(reps=reps, merge_single=merge_single, merge_steps=merge_steps)

    # ------------------ Time evolution circuit --------------------

    # 3) Build the circuit for the time evolution operator U(t) = exp(-i t H)
    evo = PauliEvolutionGate(H, time=t)
    evo_circuit = forest_ruth4.synthesize(evo)
